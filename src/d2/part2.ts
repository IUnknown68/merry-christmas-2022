...
function pointsForGame(theirPlay: string, resultCode: string): number {
  const theirNum = itemToNumber(theirPlay);
  const resultNum = itemToNumber(resultCode);
  const myNum = (theirNum + POINTS.length + resultNum - 1) % POINTS.length;
